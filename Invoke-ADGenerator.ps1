...
$banner+= $Global:Spacing + '                     Modified by Fr3akazo1d                        '
$banner+= $Global:Spacing + ''
...
wget https://github.com/dievus/PowerShellForPentesters/archive/refs/heads/main.zip -outfile C:\Shared\adgenerator.zip
...
$domainFront = $Global:domain.split('.')[0]
$domainBack = $Global:domain.split('.')[1]
New-ADUser -Name "$firstname $lastname" -GivenName $firstname -Surname $lastname -SamAccountName $SamAccountName -UserPrincipalName $principalname@$Global:Domain -Path "OU=SeniorManagement,DC=hackers,DC=com" -AccountPassword (ConvertTo-SecureString $password -AsPlainText -Force) -PassThru | Enable-ADAccount
...
New-ADUser -Name "$firstname $lastname" -GivenName $firstname -Surname $lastname -SamAccountName $SamAccountName -UserPrincipalName $principalname@$Global:Domain -Path "OU=ITAdmins,DC=hackers,DC=com" -AccountPassword (ConvertTo-SecureString $password -AsPlainText -Force) -PassThru | Enable-ADAccount
Write-Info "$samAccountName added"
...
New-ADUser -Name "$firstname $lastname" -GivenName $firstname -Surname $lastname -SamAccountName $SamAccountName -UserPrincipalName $principalname@$Global:Domain -Path "OU=Engineering,DC=hackers,DC=com" -AccountPassword (ConvertTo-SecureString $password -AsPlainText -Force) -PassThru | Enable-ADAccount
Write-Info "$samAccountName added"
...
New-ADUser -Name "$firstname $lastname" -GivenName $firstname -Surname $lastname -SamAccountName $SamAccountName -UserPrincipalName $principalname@$Global:Domain -Path "OU=Sales,DC=hackers,DC=com" -AccountPassword (ConvertTo-SecureString $password -AsPlainText -Force) -PassThru | Enable-ADAccount
Write-Info "$samAccountName added"
...
Write-Good "Adding misconfigured ACL rule for Nicholas Hathaway."
$vulnAclUser = Get-ADUser -Identity "n.hathaway"
$SourceUser = Get-ADUser -Identity "d.alderson"
AD-AddACL -Source $vulnAclUser.sid -Destination $SourceUser.DistinguishedName -Rights "GenericAll"
Write-Info "Whoops! GenericAll rights granted to n.hathaway."
